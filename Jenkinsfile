  ...
  stages {
    ...
    stage('Test') {
      steps {
          ...
          sh 'docker inspect -f \'{{if .State.Running}}{{print "Container is running."}}{{else}}{{print "Something is wrong. Container is down"}}{{exit 1}}{{end}}\' counter-service 2>/dev/null | grep -q "running" || exit 1'
          //sh './tests/tests.sh'
